...
import generateTemplateId, { HASH_PREFIX } from "./utils.js";

export default function compilerFactory(options) {
    return function (id, tokens, pathToTwig) {
        ...
        var processDependency = function (token) {
            if (token.value.indexOf(HASH_PREFIX) === 0) {
                ...
                if (typeof resolveMap[normalizedTokenValue] === "undefined") {
                    ...
                    if (resolveMap[normalizedTokenValue] === false) {
                        ...
                        token.value = generateTemplateId(
